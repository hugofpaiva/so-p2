...
{   
    ...
    // mudar o estado para PREPARING
    sh->fSt.st.agentStat = PREPARING;
    saveState(nFic, &sh->fSt);

    // generate two random ingredients to prepare
    int ing1 = rand() / (RAND_MAX / (2 + 1) + 1);
    int ing2 = rand() / (RAND_MAX / (2 + 1) + 1);

    while(ing1 == ing2){
        ing2 = rand() / (RAND_MAX / (2 + 1) + 1);
    }

    // UPDATE INVENTORY - dentro da shared memory
    ...
    /* TODO: insert your code here */
    // avisar os watchers dos dois ingredientes que ele está pronto...
...
static void waitForCigarette ()
{
    ...
    /* TODO: insert your code here */
    // update state ... waiting for the smoker to complete rolling the cigarette
    ...
    /* TODO: insert your code here */
    // QUANDO O SMOKER ACABA DE FUMAR ELE AVISA O AGENTE
    // ou seja o agente tem de fazer down para esperar por uma notificação do smoker
    
    if(semDown(semgid, sh->waitCigarette) == -1){
        // semUp para avisar o agente para este esperar
...
static void closeFactory ()
{
    ...
    /* TODO: insert your code here */
    // update state ... CLOSING_A
    sh->fSt.st.agentStat = CLOSING_A;
    sh->fSt.closing = true;
    saveState(nFic, &sh->fSt);

    // notificar todos os watchers de que a fábrica vai fechar... semUp
    // estando eles à espera de uma not ingredient, acorda-os com esta e eles verificam se está closingcd 
